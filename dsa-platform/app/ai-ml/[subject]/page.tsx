...
import { getAIMLSubject, AIML_SUBJECT_META, AIML_SUBJECTS } from "@/data/aiml";
import { useAIMLStore } from "@/lib/aimlStore";

interface Props { params: Promise<{ subject: string }>; }

export default function AIMLSubjectPage({ params }: Props) {
  const { subject: subjectId } = use(params);
  const subject = getAIMLSubject(subjectId);
  if (!subject) notFound();
  ...
  const [rightOpen, setRightOpen] = useState(() =>
    typeof window === "undefined" ? true : localStorage.getItem("aiml-right-open") !== "0");

  useEffect(() => { localStorage.setItem("aiml-left-open", leftOpen ? "1" : "0"); }, [leftOpen]);
  useEffect(() => { localStorage.setItem("aiml-right-open", rightOpen ? "1" : "0"); }, [rightOpen]);
    ...
    <div suppressHydrationWarning style={{ minHeight: "100vh", background: "var(--bg-primary)" }}>
      <main className="mx-auto max-w-[1380px] px-6 pb-8">
        <div className="flex flex-wrap items-center gap-2 pb-8 pt-8 text-xs" style={{ color: "var(--text-muted)" }}>
          <Link href="/ai-ml" className="hover:underline">AI &amp; ML</Link>
          <span>/</span>
        ...
        <div className={`grid justify-center gap-14 ${gridClass}`}>
          ...
          <article className="prose-se w-full min-w-0" key={activeId}>
            {activeChapter && (
              <>
                <div className="reveal reveal-1">
                  <div className="mb-3 text-xs font-medium" style={{ color: meta?.accent ?? "var(--accent)" }}>
                    Chapter {activeChapter.num}
                  </div>
                  <h1 className="mb-10 text-4xl font-semibold leading-tight tracking-tight" style={{ color: "var(--text-primary)" }}>
                    {activeChapter.title}
                  </h1>
                </div>
                  ...
                  <div>
                    ...
                    <div className="mb-4 flex items-center gap-3">
                      <div className="h-px flex-1 overflow-hidden" style={{ background: "var(--border-subtle)" }}>
                        <div className="h-full transition-all" style={{ width: `${(doneCount / subject.chapters.length) * 100}%`, background: meta?.accent ?? "var(--accent)" }} />
                      </div>
                      <span className="text-xs" style={{ color: "var(--text-muted)" }}>{doneCount}/{subject.chapters.length}</span>
                    </div>
                    <button onClick={() => toggleChapter(key)} className="text-xs font-medium transition-colors"
                      style={{ color: done ? "var(--accent-green)" : meta?.accent ?? "var(--accent)" }}>
                      ...
                      <ul className="space-y-3">
                        {takeaways.map((t, i) => (
                          <li key={i} className="flex gap-2 text-xs leading-6" style={{ color: "var(--text-secondary)" }}>
                            <span style={{ color: meta?.accent ?? "var(--accent)" }}>-</span><span>{t}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {interviewCount > 0 && (
                    <div>
                      ...
                      <p className="text-xs leading-6" style={{ color: "var(--text-secondary)" }}>
                        {interviewCount} interview question{interviewCount > 1 ? "s" : ""} in this chapter.
                      </p>
                    </div>
                  )}

                  <div>
                    <div className="mb-3 text-[11px] font-medium" style={{ color: "var(--text-muted)" }}>All sections</div>
                    <div className="space-y-1">
                      {AIML_SUBJECTS.map((s) => (
                        <Link key={s.id} href={`/ai-ml/${s.id}`}
                          className="block text-xs py-1 transition-colors"
                          style={{ color: s.id === subjectId ? "var(--accent)" : "var(--text-muted)", fontWeight: s.id === subjectId ? 600 : 400 }}>
                          {s.title}
                        </Link>
                      ))}
                    </div>
...
function ChapterNav({ subject, activeId, dir, onGo }: {
  subject: ReturnType<typeof getAIMLSubject>; activeId: string; dir: -1 | 1; onGo: (id: string) => void;
}) {
