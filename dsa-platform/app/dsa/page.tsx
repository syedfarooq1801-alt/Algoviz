  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg-primary)", display: "flex", flexDirection: "column" }}>
      <div style={{
        ...
        height: isMobile ? undefined : "calc(100vh - 0px)",
      }}>
        ...
        {!isMobile && (
          <aside style={{
            ...
            <button onClick={() => setActiveTopic("all")} style={{
              ...
              background: activeTopic === "all" ? "var(--accent-soft)" : "transparent",
              color: activeTopic === "all" ? "var(--accent)" : "var(--text-secondary)",
              borderRadius: 5, marginBottom: 1, fontWeight: activeTopic === "all" ? 600 : 400,
              outline: activeTopic === "all" ? "1px solid rgba(79,140,255,0.18)" : "1px solid transparent",
              transition: "all 0.1s",
            }}>
              <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>All Topics</span>
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, opacity: 0.55, flexShrink: 0, marginLeft: 4 }}>{solved.size}/{total}</span>
            </button>
            {PATTERNS.map((p) => {
        ...
        {/* ── Main column ── */}
        <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, overflow: isMobile ? "visible" : "hidden" }}>
          ...
          {isMobile && (
            <div style={{
              ...
              {[{ id: "all", title: "All Topics" }, ...PATTERNS].map((p) => {
                const active = activeTopic === p.id;
                return (
                  <button key={p.id} onClick={() => setActiveTopic(p.id)} style={{
                    flexShrink: 0, padding: "5px 13px", fontSize: 12, borderRadius: 20, cursor: "pointer",
                    background: active ? "var(--accent-soft)" : "transparent",
                    color: active ? "var(--accent)" : "var(--text-secondary)",
                    border: active ? "1px solid rgba(79,140,255,0.3)" : "1px solid var(--border-subtle)",
                    fontWeight: active ? 600 : 400, whiteSpace: "nowrap", transition: "all 0.1s",
                  }}>{p.title}</button>
                );
              })}
            </div>
          ...
          {/* Filter bar */}
          <div style={{ borderBottom: "1px solid var(--border-subtle)", background: "var(--bg-primary)", flexShrink: 0 }}>
            {/* Search row */}
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: isMobile ? "8px 14px" : "8px 20px" }}>
              <input
                value={search}
                ...
                style={{
                  ...
                  color: "var(--text-secondary)", padding: "5px 10px", borderRadius: 6,
            ...
            {/* Difficulty row */}
            <div style={{ display: "flex", alignItems: "center", gap: 3, padding: isMobile ? "0 14px 8px" : "0 20px 8px" }}>
              {(["All", "Easy", "Medium", "Hard"] as Diff[]).map((d) => (
                <button key={d} onClick={() => setDiff(d)} style={{
                  padding: isMobile ? "3px 10px" : "4px 12px", fontSize: 12, borderRadius: 5, cursor: "pointer",
                  ...
                  border: diff === d ? `1px solid ${d === "All" ? "rgba(79,140,255,0.22)" : DIFF_COLOR[d] + "55"}` : "1px solid transparent",
                  transition: "all 0.1s",
                }}>{d}</button>
              ))}
            </div>
            {/* Company row */}
            <div style={{
              display: "flex", gap: 4, padding: isMobile ? "0 14px 10px" : "0 20px 8px",
              overflowX: isMobile ? "auto" : undefined, flexWrap: isMobile ? undefined : "wrap",
              scrollbarWidth: "none",
            }}>
              {COMPANIES.map((c) => {
                ...
                return (
                  <button key={c} onClick={() => setCompany(c)} style={{
                    flexShrink: 0, padding: "3px 10px", fontSize: 11, borderRadius: 20, cursor: "pointer",
                    fontWeight: active ? 600 : 400,
                    background: active ? (c === "All" ? "var(--accent-soft)" : color + "22") : "transparent",
                    color: active ? (c === "All" ? "var(--accent)" : color) : "var(--text-muted)",
                    border: active ? `1px solid ${c === "All" ? "rgba(79,140,255,0.3)" : color + "55"}` : "1px solid var(--border-subtle)",
          ...
          {activeTopic !== "all" && (
            <div style={{
              ...
              borderBottom: "1px solid var(--border-subtle)", background: "rgba(79,140,255,0.04)",
            }}>
          ...
          {/* Table header */}
          <div style={{
            display: "grid", gridTemplateColumns: tableGrid,
          ...
          <div style={{ flex: 1, overflowY: isMobile ? "visible" : "auto" }}>
            {problems.map((prob, i) => {
              ...
              const companies = (prob.companies ?? []).slice(0, isMobile ? 2 : 3);
              ...
              return (
                <div key={prob.id} style={{
                  ...
                  minHeight: companies.length > 0 ? (isMobile ? 52 : 58) : (isMobile ? 40 : 44),
                }}>
                  ...
                  <div style={{ minWidth: 0, paddingRight: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                      ...
                      {isDue && (
                        <span style={{ fontSize: 9, padding: "1px 4px", borderRadius: 3, flexShrink: 0, background: "rgba(245,165,36,0.15)", color: "#F5A524", fontFamily: "var(--font-mono)" }}>DUE</span>
                      )}
                    </div>
                    {companies.length > 0 && (
                  ...
                  {!isMobile && (
                    ...
                    }}>{pattern?.title ?? prob.pattern}</span>
                  )}

                  {/* Difficulty */}
                  <span style={{
                    display: "inline-flex", alignItems: "center",
                    fontSize: isMobile ? 10 : 11, fontWeight: 600, fontFamily: "var(--font-mono)",
                    padding: isMobile ? "2px 6px" : "3px 8px", borderRadius: 4,
                    color: DIFF_COLOR[prob.difficulty], background: DIFF_BG[prob.difficulty],
                  }}>
                  ...
                  <button onClick={() => toggleSolved(prob.id)} title={isSolved ? "Mark unsolved" : "Mark solved"}
                    style={{ display: "flex", alignItems: "center", justifyContent: "center", background: "none", border: "none", cursor: "pointer", padding: 0, width: "100%" }}>
                    <span style={{
                  ...
                  {!isMobile && (
                    <button onClick={(e) => { e.stopPropagation(); toggleBookmark(prob.id); }} title={bookmarked ? "Remove bookmark" : "Bookmark"}
                      style={{
                        display: "flex", alignItems: "center", justifyContent: "center",
                        background: "none", border: "none", cursor: "pointer", padding: "0 4px",
                        fontSize: 14, color: bookmarked ? "#F5A524" : "var(--text-muted)",
                        opacity: bookmarked ? 1 : 0.45, transition: "color 0.15s, opacity 0.15s",
                      }}
