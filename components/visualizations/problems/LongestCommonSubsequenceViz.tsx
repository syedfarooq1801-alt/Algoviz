...
export default function LongestCommonSubsequenceViz() {
  const [s1, setS1] = useState("abcde");
  const [s2, setS2] = useState("ace");
  const [i1, setI1] = useState("abcde");
  const [i2, setI2] = useState("ace");
  ...
  const [msg, setMsg] = useState("Press Play — dp[i][j] = LCS of s1[0..i] and s2[0..j]");
  const stateRef = useRef({ r:1, c:1, dp:[] as number[][], s1:"abcde", s2:"ace" });
  const iRef = useRef<ReturnType<typeof setInterval>|null>(null);

  const reset = (a=s1, b=s2) => {
    const m=a.length, n=b.length;
    const init = Array(m+1).fill(null).map(()=>Array(n+1).fill(0));
    stateRef.current = { r:1, c:1, dp:init, s1:a, s2:b };
    setDp(init.map(r=>[...r])); setActiveR(-1); setActiveC(-1); setDone(false); setPlaying(false);
    setMsg("dp[0][*]=0 and dp[*][0]=0 (empty string has LCS=0)"); if (iRef.current) clearInterval(iRef.current);
  };

  const doStep = () => {
    const { r, c, dp: d, s1: a, s2: b } = stateRef.current;
    if (r > a.length) { setDone(true); setPlaying(false); setActiveR(-1); setMsg(`Done! LCS length = ${stateRef.current.dp[a.length][b.length]}`); return; }
    const nd = d.map(row=>[...row]);
    if (a[r-1] === b[c-1]) { nd[r][c] = d[r-1][c-1]+1; }
    else { nd[r][c] = Math.max(d[r-1][c], d[r][c-1]); }
    ...
    if (a[r-1]===b[c-1]) setMsg(`s1[${r-1}]='${a[r-1]}'==s2[${c-1}]='${b[c-1]}' → dp[${r}][${c}]=dp[${r-1}][${c-1}]+1=${nd[r][c]}`);
    else setMsg(`s1[${r-1}]='${a[r-1]}'≠s2[${c-1}]='${b[c-1]}' → dp[${r}][${c}]=max(${d[r-1][c]},${d[r][c-1]})=${nd[r][c]}`);
  };
  ...
  const apply = () => { setS1(i1); setS2(i2); reset(i1,i2); };

  return (
    <div className="space-y-4">
      <div className="rounded-xl p-4" style={{ background:"var(--bg-card)", border:"1px solid var(--border)" }}>
        <h3 className="text-sm font-semibold mb-3" style={{ color:"var(--text-primary)" }}>Longest Common Subsequence — 2D DP</h3>
        <div className="flex flex-wrap gap-3 mb-3">
          <div className="flex items-center gap-2"><label className="text-xs" style={{ color:"var(--text-muted)" }}>s1:</label>
          ...
          <div className="flex items-center gap-2"><label className="text-xs" style={{ color:"var(--text-muted)" }}>s2:</label>
      ...
      <div className="rounded-xl p-4 overflow-x-auto" style={{ background:"var(--bg-card)", border:"1px solid var(--border)" }}>
        <table style={{ borderCollapse:"separate", borderSpacing:"3px" }}>
          ...
          <tbody>
            {dp.map((row,r)=>(
              <tr key={r}>
                <td style={{ fontSize:"10px", color:r===0?"var(--text-muted)":"#f97316", padding:"2px", textAlign:"right", paddingRight:"4px" }}>{r===0?'""':s1[r-1]}</td>
                {row.map((v,c)=>{
                  const isActive = r===activeR&&c===activeC;
                  const isResult = done&&r===s1.length&&c===s2.length;
                  return (
                    <td key={c} style={{ padding:0 }}>
                      <div className="w-9 h-9 rounded flex items-center justify-center text-xs font-bold font-mono transition-all duration-200"
                        style={{ background:isResult?"rgba(34,197,94,0.3)":isActive?"rgba(79,142,247,0.3)":r===0||c===0?"rgba(255,255,255,0.04)":v>0?"rgba(168,85,247,0.1)":"var(--bg-hover)", border:isResult?"2px solid #22c55e":isActive?"2px solid #4f8ef7":"1px solid var(--border)", color:isResult?"#22c55e":isActive?"#4f8ef7":v>0?"#a855f7":"var(--text-muted)", transform:isActive?"scale(1.1)":"scale(1)" }}>
                        {v}
                      </div>
                    </td>
                  );
      ...
      {done && dp.length && (
        <div className="rounded-xl p-4 text-center" style={{ background:"rgba(34,197,94,0.08)", border:"1px solid rgba(34,197,94,0.3)" }}>
          <div className="font-semibold text-sm" style={{ color:"#22c55e" }}>LCS length: {dp[s1.length]?.[s2.length] ?? 0}</div>
        </div>
      )}
    </div>
  );
